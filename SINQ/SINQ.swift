...
//    open func selectMany<S: Sequence, R>(_ selector: @escaping (T) -> S, result: (S.Iterator.Element) -> R) -> SinqSequence<R> {
//        return selectMany({ (x, _) in selector(x) }, result: result)
//    }
//
//    open func selectMany<S: Sequence>(_ selector: (T, Int) -> S) -> SinqSequence<S.Iterator.Element> {
